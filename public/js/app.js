...
const elements = {
  ...
  // Elementi Barra Navigazione Desktop
  desktopClassName: document.getElementById('desktop-class-name'),
  desktopClassBadge: document.getElementById('desktop-class-badge'),
  btnDesktopChange: document.getElementById('btn-desktop-change'),
  btnDesktopOriginal: document.getElementById('btn-desktop-original'),
  btnDesktopSync: document.getElementById('btn-desktop-sync'),
  btnDesktopPrint: document.getElementById('btn-desktop-print'),
  btnDesktopShortcuts: document.getElementById('btn-desktop-shortcuts'),
  btnDesktopSettings: document.getElementById('btn-desktop-settings'),
  desktopClockTime: document.getElementById('desktop-clock-time'),

  // Pannello Companion Desktop (Vista Giorno)
  dailyCompanionPanel: document.getElementById('daily-companion-panel'),
  companionClassName: document.getElementById('companion-class-name'),
  companionStatusBadge: document.getElementById('companion-status-badge'),
  btnCompanionChangeClass: document.getElementById('btn-companion-change-class'),
  companionDateDecorrenza: document.getElementById('companion-date-decorrenza'),
  companionLiveCard: document.getElementById('companion-live-card'),
  companionLiveStatusTitle: document.getElementById('companion-live-status-title'),
  companionLiveSubtitle: document.getElementById('companion-live-subtitle'),
  companionProgressFill: document.getElementById('companion-progress-fill'),
  companionNextValue: document.getElementById('companion-next-value'),
  companionDaysList: document.getElementById('companion-days-list'),
  companionSummaryTitle: document.getElementById('companion-summary-title'),
  companionStatHours: document.getElementById('companion-stat-hours'),
  companionStatLabs: document.getElementById('companion-stat-labs'),
  companionStatBreaks: document.getElementById('companion-stat-breaks'),
  btnCompanionOpenShortcuts: document.getElementById('btn-companion-open-shortcuts'),
...
/**
 * Aggiorna gli indicatori di classe e stato nell'Header Desktop
 */
function updateDesktopHeader() {
  const currentClassName = state.timetable?.classe || '4 BINF';
  if (elements.desktopClassName) {
    elements.desktopClassName.textContent = currentClassName;
  }
}

/**
 * Renderizza e sincronizza il Companion Panel Desktop (Vista Giorno a 2 colonne)
 */
function renderDesktopCompanionPanel() {
  if (!state.timetable) return;

  const currentClassName = state.timetable.classe || '4 BINF';
  if (elements.companionClassName) elements.companionClassName.textContent = currentClassName;
  if (elements.desktopClassName) elements.desktopClassName.textContent = currentClassName;

  if (elements.companionStatusBadge) {
    elements.companionStatusBadge.textContent = state.timetable.stato_orario || 'Orario Provvisorio';
  }

  if (elements.companionDateDecorrenza) {
    elements.companionDateDecorrenza.textContent = state.timetable.data_decorrenza || `Aggiornato: ${state.timetable.data_aggiornamento || ''}`;
  }

  // 1. Popola Navigatore Giorni Settimana Companion
  if (elements.companionDaysList) {
    elements.companionDaysList.innerHTML = '';
    ...
    giorni.forEach(day => {
      ...
      const isActive = day.giorno === state.selectedDay;
      const isToday = day.giorno === todayName;
      row.className = `companion-day-nav-row ${isActive ? 'active' : ''}`;
      row.setAttribute('data-day', day.giorno);

      const numLessons = (day.lezioni || []).length;
      const dateStr = getFormattedDateForDay(day.giorno, now).replace(day.giorno.toUpperCase(), '').trim();

      row.innerHTML = `
        <div class="companion-day-info">
          <span class="companion-day-name">${escapeHtml(day.giorno)} ${isToday ? '<span style="color:var(--live-line-color);font-size:0.68rem;font-weight:800;">(Oggi)</span>' : ''}</span>
          <span class="companion-day-date">${escapeHtml(dateStr)}</span>
        </div>
        <span class="companion-day-badge">${numLessons} ore</span>
      `;

      row.addEventListener('click', () => {
        if (day.giorno !== state.selectedDay) {
          state.selectedDay = day.giorno;
          render();
        }
      });

      elements.companionDaysList.appendChild(row);
    });
  }

  // 2. Scheda Statistiche del giorno selezionato
  const dayData = (state.timetable.giorni || []).find(d => d.giorno === state.selectedDay);
  if (elements.companionSummaryTitle) {
    elements.companionSummaryTitle.textContent = `RIEPILOGO ${state.selectedDay.toUpperCase()}`;
  }

  if (dayData && dayData.lezioni) {
    const totalHours = dayData.lezioni.length;
    const labHours = dayData.lezioni.filter(l => l.is_lab || (l.materia && l.materia.toUpperCase().includes('LAB'))).length;
    
    const structure = extractTimetableStructure(state.timetable);
    const breakCount = structure.filter(s => s.type === 'break').length || 1;

    if (elements.companionStatHours) elements.companionStatHours.textContent = String(totalHours);
    if (elements.companionStatLabs) elements.companionStatLabs.textContent = String(labHours);
    if (elements.companionStatBreaks) elements.companionStatBreaks.textContent = String(breakCount);
  } else {
    if (elements.companionStatHours) elements.companionStatHours.textContent = '0';
    if (elements.companionStatLabs) elements.companionStatLabs.textContent = '0';
    if (elements.companionStatBreaks) elements.companionStatBreaks.textContent = '0';
  }

  // 3. Aggiorna stato live del Companion Panel
  updateCompanionLiveStatus(getCurrentDate());
}

/**
 * Aggiorna il box live status (countdown, barra progresso, prossima ora) nel Companion Panel
 */
function updateCompanionLiveStatus(now = new Date()) {
  if (!elements.companionLiveStatusTitle || !state.timetable) return;

  const dayOfWeek = now.getDay(); // 0 = Dom, 1 = Lun, ..., 6 = Sab
  const dayNames = ['Domenica', 'Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato'];
  const todayName = dayNames[dayOfWeek];
  const todayData = (state.timetable.giorni || []).find(d => d.giorno === todayName);

  if (!todayData || !todayData.lezioni || todayData.lezioni.length === 0) {
    elements.companionLiveStatusTitle.textContent = "Nessuna lezione oggi";
    if (elements.companionLiveSubtitle) elements.companionLiveSubtitle.textContent = "Nessun orario previsto per la data odierna";
    if (elements.companionProgressFill) elements.companionProgressFill.style.width = '0%';
    if (elements.companionNextValue) elements.companionNextValue.textContent = "Riposo / Weekend";
    return;
  }

  const currentMinutes = now.getHours() * 60 + now.getMinutes();
  const status = evaluateCurrentStatus(todayData.lezioni, currentMinutes);

  if (status.type === 'BEFORE_SCHOOL') {
    elements.companionLiveStatusTitle.textContent = "Prima delle lezioni";
    if (elements.companionLiveSubtitle) {
      elements.companionLiveSubtitle.textContent = `Inizio alle ${status.firstLesson.inizio} (tra ${status.minutesUntilStart} min)`;
    }
    if (elements.companionProgressFill) elements.companionProgressFill.style.width = '0%';
    if (elements.companionNextValue) {
      elements.companionNextValue.textContent = `1ª Ora: ${status.firstLesson.materia} (${status.firstLesson.aula})`;
    }
  } else if (status.type === 'IN_LESSON') {
    elements.companionLiveStatusTitle.textContent = `🟢 In corso: ${status.lesson.materia}`;
    if (elements.companionLiveSubtitle) {
      elements.companionLiveSubtitle.textContent = `Aula ${status.lesson.aula} • Termina tra ${status.minutesRemaining} min (${status.lesson.fine})`;
    }
    if (elements.companionProgressFill) {
      elements.companionProgressFill.style.width = `${Math.min(100, Math.max(0, status.progressPercent))}%`;
    }
    if (elements.companionNextValue) {
      if (status.nextLesson) {
        elements.companionNextValue.textContent = `${status.nextLesson.materia} (${status.nextLesson.aula}) alle ${status.nextLesson.inizio}`;
      } else {
        elements.companionNextValue.textContent = "Uscita da scuola al termine dell'ora";
      }
    }
  } else if (status.type === 'BREAK') {
    elements.companionLiveStatusTitle.textContent = "🔔 Ricreazione in corso";
    if (elements.companionLiveSubtitle) {
      elements.companionLiveSubtitle.textContent = `Ripresa lezioni tra ${status.minutesUntilNext} min alle ${status.nextLesson.inizio}`;
    }
    if (elements.companionProgressFill) elements.companionProgressFill.style.width = '50%';
    if (elements.companionNextValue) {
      elements.companionNextValue.textContent = `${status.nextLesson.materia} (${status.nextLesson.aula})`;
    }
  } else if (status.type === 'FINISHED') {
    elements.companionLiveStatusTitle.textContent = "Lezioni terminate";
    if (elements.companionLiveSubtitle) {
      elements.companionLiveSubtitle.textContent = "Tutte le lezioni di oggi sono concluse";
    }
    if (elements.companionProgressFill) elements.companionProgressFill.style.width = '100%';
    if (elements.companionNextValue) {
      elements.companionNextValue.textContent = "Ci vediamo domani!";
    }
  } else {
    elements.companionLiveStatusTitle.textContent = "Orario scolastico";
    if (elements.companionLiveSubtitle) elements.companionLiveSubtitle.textContent = "Visualizzazione attiva";
    if (elements.companionProgressFill) elements.companionProgressFill.style.width = '0%';
    if (elements.companionNextValue) elements.companionNextValue.textContent = "--";
  }
}

/**
 * Aggiorna i metadati visualizzati nell'intestazione di stampa
 */
function updatePrintMetadata() {
  const printDocTitle = document.getElementById('print-doc-title');
  const printClassBadge = document.getElementById('print-class-badge');
  const printDateInfo = document.getElementById('print-date-info');
  const printTimestamp = document.getElementById('print-timestamp');

  const className = state.timetable?.classe || '4 BINF';
  const now = new Date();
  const dateStr = now.toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric' });
  const timeStr = now.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' });

  if (printDocTitle) printDocTitle.textContent = "ORARIO SCOLASTICO SETTIMANALE";
  if (printClassBadge) printClassBadge.textContent = `CLASSE: ${className}`;
  if (printDateInfo) printDateInfo.textContent = state.timetable?.data_decorrenza || 'Anno Scolastico in corso';
  if (printTimestamp) printTimestamp.textContent = `Stampato il ${dateStr} alle ${timeStr}`;
}

/**
 * Avvia la procedura di stampa ottimizzata per PC (formato A4)
 */
function triggerPrint() {
  if (state.currentView === 'change-schedule' || state.currentView === 'original-schedule') {
    returnToScheduleView();
  }
  
  state.currentView = 'weekly';
  if (elements.btnViewWeekly) elements.btnViewWeekly.classList.add('active');
  if (elements.btnViewDaily) elements.btnViewDaily.classList.remove('active');
  render();
  ...
  if (elements.modalShortcuts) elements.modalShortcuts.classList.remove('open');
  if (elements.modalSettings) elements.modalSettings.classList.remove('open');
  if (elements.modalWidget) elements.modalWidget.classList.remove('open');
...
function tick() {
  ...
  // Aggiorna orologio digitale desktop
  if (elements.desktopClockTime) {
    const secStr = String(now.getSeconds()).padStart(2, '0');
    const minStr = String(now.getMinutes()).padStart(2, '0');
    const hrStr = String(now.getHours()).padStart(2, '0');
    elements.desktopClockTime.textContent = `${hrStr}:${minStr}:${secStr}`;
  }

  // Aggiorna stato live del Companion Panel desktop
  updateCompanionLiveStatus(now);
...
function setupEventListeners() {
  ...
  // Azioni Header Desktop e Companion Panel
  if (elements.btnDesktopChange) {
    elements.btnDesktopChange.addEventListener('click', () => {
      openChangeScheduleView('image');
    });
  }

  if (elements.btnDesktopOriginal) {
    elements.btnDesktopOriginal.addEventListener('click', () => {
      openOriginalScheduleView();
    });
  }

  if (elements.btnDesktopSync) {
    elements.btnDesktopSync.addEventListener('click', () => {
      triggerSync();
    });
  }

  if (elements.btnDesktopPrint) {
    elements.btnDesktopPrint.addEventListener('click', () => {
      triggerPrint();
    });
  }

  if (elements.btnDesktopShortcuts) {
    elements.btnDesktopShortcuts.addEventListener('click', () => {
      if (elements.modalShortcuts) elements.modalShortcuts.classList.toggle('open');
    });
  }

  if (elements.btnCompanionOpenShortcuts) {
    elements.btnCompanionOpenShortcuts.addEventListener('click', () => {
      if (elements.modalShortcuts) elements.modalShortcuts.classList.add('open');
    });
  }

  if (elements.btnCloseModalShortcuts) {
    elements.btnCloseModalShortcuts.addEventListener('click', () => {
      if (elements.modalShortcuts) elements.modalShortcuts.classList.remove('open');
    });
  }
  ...
  if (elements.modalShortcuts) {
    elements.modalShortcuts.addEventListener('click', (e) => {
      if (e.target === elements.modalShortcuts) {
        elements.modalShortcuts.classList.remove('open');
      }
    });
  }

  if (elements.btnDesktopSettings) {
    elements.btnDesktopSettings.addEventListener('click', () => {
      if (elements.modalSettings) elements.modalSettings.classList.add('open');
    });
  }

  if (elements.desktopClassBadge) {
    elements.desktopClassBadge.addEventListener('click', () => {
      openChangeScheduleView('preset');
    });
  }

  if (elements.btnCompanionChangeClass) {
    elements.btnCompanionChangeClass.addEventListener('click', () => {
      openChangeScheduleView('preset');
    });
  }

  // Registra le scorciatoie da tastiera globali per PC
  setupKeyboardShortcuts();

  /**
   * Configura le scorciatoie da tastiera per navigazione ultra-veloce da PC
   */
  function setupKeyboardShortcuts() {
    window.addEventListener('keydown', (e) => {
      // Non intercettare se l'utente sta digitando in un campo testo
      ...
      // Ctrl+P o standalone P: Stampa orario
      ...
      if (e.key === 'Escape') {
        if (elements.modalShortcuts && elements.modalShortcuts.classList.contains('open')) {
          elements.modalShortcuts.classList.remove('open');
          return;
        }
        if (elements.modalSettings && elements.modalSettings.classList.contains('open')) {
          elements.modalSettings.classList.remove('open');
          return;
        }
        if (elements.modalWidget && elements.modalWidget.classList.contains('open')) {
          elements.modalWidget.classList.remove('open');
          return;
        }
        if (elements.sidebarDrawer && elements.sidebarDrawer.classList.contains('open')) {
      ...
      if (e.key === 't' || e.key === 'T') {
        e.preventDefault();
        const now = getCurrentDate();
        const todayName = getSmartDefaultDay(state.timetable?.giorni, now);
        if (todayName) {
      ...
      if (e.key === 'o' || e.key === 'O') {
        e.preventDefault();
        if (state.currentView === 'original-schedule') {
          returnToScheduleView();
        } else {
          openOriginalScheduleView();
        }
      ...
      if (e.key === 'm' || e.key === 'M') {
        e.preventDefault();
        const isOpen = elements.sidebarDrawer?.classList.contains('open');
        toggleDrawer(!isOpen);
        return;
      }
      ...
      if (e.key === '?') {
        e.preventDefault();
        if (elements.modalShortcuts) {
          elements.modalShortcuts.classList.toggle('open');
        }
        return;
      }
    });
  ...
  // Ricalibra cursore e layout al ridimensionamento della finestra su PC / browser
